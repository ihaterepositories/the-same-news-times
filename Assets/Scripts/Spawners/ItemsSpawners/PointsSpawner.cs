...
namespace Spawners.ItemsSpawners
{
    ...
    {
        private ObjectPool<Point> _pool;
        ...
        private void Awake()
        {
            _pool = new ObjectPool<Point>(_prefabsLoader.GetPrefab("Point").GetComponent<Point>());
        }
        ...
        public void Spawn(Cell[,] maze, int mazeWidth, int mazeHeight, int iterationsCoefficient = 1)
        {
            ...
            for (var i = 0; i < iterations; i++)
            {
                    ...
                    _positionsBlocker.CheckPositionAvailability(xPosition, yPosition))
                {
                    ...
                    _positionsBlocker.Block(xPosition, yPosition, false);
                }
            }
        }
        ...
        private Point GetGreenScoreObject()
        {
            var poolAble = _pool.GetFreeObject();
            return poolAble as Point;
        }
    }
}
